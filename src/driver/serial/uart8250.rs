...
impl Uart {
    ...
    pub unsafe fn new(
        ...
        Self {
            ...
            reg_io_width,
            reg_shift: (1 << reg_shift) / reg_io_width - 1,
        }
    }
    ...
    fn init_u8(&mut self) {
        ...
        unsafe {
            // Disable Interrupt
            reg.add(IER << self.reg_shift).write_volatile(0x00);

            // Enable DLAB
            // Enter a setting mode to set baud rate
            reg.add(LCR << self.reg_shift).write_volatile(0x80);

            // Set baud rate
            let divisor = self.clock_frequency / (16 * self.baud_rate);
            reg.add(0 << self.reg_shift).write_volatile(divisor as u8);
            reg.add(1 << self.reg_shift).write_volatile((divisor >> 8) as u8);

            // Disable DLAB and set data word length to 8 bits
            // Leave setting mode
            reg.add(LCR << self.reg_shift).write_volatile(0x03);

            // Enable FIFO
            reg.add(FCR << self.reg_shift).write_volatile(0x01);

            // No modem control
            reg.add(MCR << self.reg_shift).write_volatile(0x00);

            // Enable interrupts now
            reg.add(IER << self.reg_shift).write_volatile(0x01);
        }
    }
    ...
    fn init_u32(&mut self) {
        ...
        unsafe {
            ...
            // Set baud rate
            let divisor = self.clock_frequency / (16 * self.baud_rate);
            reg.add(0 << self.reg_shift).write_volatile(divisor & 0xff);
            reg.add(1 << self.reg_shift).write_volatile((divisor >> 8) & 0xff);
        ...
        unsafe {
            match c {
                8 | 0x7F => {
                    ...
                    ptr.add(THR << self.reg_shift).write_volatile(8);
                    wait_for!(self.line_sts_u32().contains(LineStsFlags::OUTPUT_EMPTY));
                    ptr.add(THR << self.reg_shift).write_volatile(b' '.into());
                    wait_for!(self.line_sts_u32().contains(LineStsFlags::OUTPUT_EMPTY));
                    ptr.add(THR << self.reg_shift).write_volatile(8);
                }
                _ => {
                    ...
                    wait_for!(self.line_sts_u32().contains(LineStsFlags::OUTPUT_EMPTY));
                    // Write data
                    ptr.add(THR << self.reg_shift).write_volatile(c.into());
                }
            }
        }
    }
