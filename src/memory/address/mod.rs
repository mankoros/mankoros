...
pub fn kernel_phys_to_virt(addr: usize) -> usize {
    ...
    let offset = addr.checked_sub(consts::platform::phymem_start());
    if offset.is_none() {
    ...
    if virt_addr.is_none() {
        debug!("phymem_start: 0x{:x}", consts::platform::phymem_start());
...
pub fn kernel_virt_text_to_phys(addr: usize) -> usize {
    if addr < consts::address_space::K_SEG_TEXT_BEG {
        warn!("Virtual address 0x{:x} is not in kernel text segment", addr);
        return addr;
    }
    addr - consts::address_space::K_SEG_TEXT_BEG + consts::platform::platform_boot_pc()
}
...
pub fn kernel_virt_to_phys(addr: usize) -> usize {
    // Return if the address is obviously in HIGH address space
    if addr <= consts::address_space::K_SEG_PHY_MEM_BEG {
        warn!(
    ...
    addr - consts::address_space::K_SEG_PHY_MEM_BEG + consts::platform::phymem_start()
}
...
pub fn kernel_phys_dev_to_virt(addr: usize) -> usize {
    ...
    addr - consts::platform::DEVICE_START + consts::address_space::K_SEG_HARDWARE_BEG
}
