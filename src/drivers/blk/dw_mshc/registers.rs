...
/// When a response is received – either erroneous or valid – the DWC_mobile_storage sets the command_done bit in the RINTSTS register.
/// A short response is copied in Response Register0, while a long response is copied to all four response registers @0x30, 0x34, 0x38, and 0x3C.
/// The Response3 register bit 31 represents the MSB, and the Response0 register bit 0 represents the LSB of a long response.
#[bitfield(u32)]
pub struct CMD {
    #[bits(6)]
    pub cmd_index: usize,
...
impl CMD {
    ...
    /// the DWC_mobile_storage does not raise a command_done signal upon command completion.
...
/// assigned to it. For example, bits[1:0] assigned for card-0, which maps and internally routes clock divider[3:0] outputs to cclk_out[15:0] pins, depending on bit value.
/// 00 - Clock divider 0
/// 01 - Clock divider 1
#[bitfield(u32)]
pub struct CLKSRC {
